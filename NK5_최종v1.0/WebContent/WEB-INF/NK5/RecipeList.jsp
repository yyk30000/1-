...
<%@ page language="java" contentType="text/html; charset=utf-8"
    pageEncoding="utf-8"%>
<%
List<RecipeBasicsVO> RBvo = (List<RecipeBasicsVO>) request.getAttribute("List");
String cpath = request.getContextPath();
request.setCharacterEncoding("utf-8");
response.setContentType("text/html; charset=utf-8");
MemberVO m =(MemberVO) session.getAttribute("succ");
List<RecipeBasicsVO> mylist = (List<RecipeBasicsVO>)request.getAttribute("name");
%>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <script src="https://use.fontawesome.com/releases/v5.2.0/js/all.js"></script><!--합체!-->
  <!-- 구글폰트 -->
...
function ListAll(){
	location.href = "<%=cpath%>/RecipesKategorie.do?key=0"
}
function ListKorean(){
	location.href = "<%=cpath%>/RecipesKategorie.do?key=1"
}
function ListAsia(){
	location.href = "<%=cpath%>/RecipesKategorie.do?key=2"
}
function ListWestern(){
	location.href = "<%=cpath%>/RecipesKategorie.do?key=3"
}
function ListFusion(){
	location.href = "<%=cpath%>/RecipesKategorie.do?key=4"
}
...
function Insertmyrecipe(result) {
			location.href = "<%=cpath%>/InsertMyRecipes.do?key=2&rcp_code="+result;
			
	}

	function Deletmyrecipe(result) {
		location.href = "<%=cpath%>/DeletMyRecipes.do?key=2&rcp_code="+result;
...
<!--(start)검색창 -->
	<div class="jumbotron text-center">
		...
		<h1 style="font-family: 'EliceDigitalBaeum_Bold'">오냉끼</h1>
...
<!-- Container (About Section) -->
  ...
  <h2>Recipe search results</h2>
    ...
    <p id="line"></p>
      <div class="container">
        <button type="button" class="btn btn-default" onclick="ListAll()">All</button>
        <button type="button" class="btn btn-default" onclick="ListKorean()">한식</button>
        <button type="button" class="btn btn-default" onclick="ListAsia()">아시아</button>
        <button type="button" class="btn btn-default" onclick="ListWestern()">서양</button>
        <button type="button" class="btn btn-default" onclick="ListFusion()">퓨전</button>
      </div>      
	  <br>
      <div class="row text-center">
        <%
					int p = 1;
				%>
       <%for(RecipeBasicsVO vo : RBvo){ %>
        <div class="col-sm-4">
          <div class="thumbnail">
            <a href="<%=cpath%>/SelectRecipePage.do?code=<%=vo.getRcp_code()%>" style="color: black" text-decoration-line: none><img src="<%=vo.getRcp_pic1()%>" alt="" width="230" height="230">
            <p class="font-rcp"><strong><%=vo.getRcp_name()%></strong></p>
            <p class="font-rcp-desc"><%=vo.getRcp_desc() %></p></a>
            <!-- 합체! -->
            
            <%if(m == null){ %>
						<button id="rcpickStar<%=p%>" class="btn btn-lg myrecipick" onclick="alert_login()">Pick　등록 <i class="fas fa-star"></i></button>
						
           	
           	<%}else{
           		Boolean check =false;
           		for(RecipeBasicsVO vo2 : mylist){
           			if(vo.getRcp_code() ==vo2.getRcp_code()){
           				check =true;
           			}
           		}%>
           			<% if(!(check)){%>
           				<button id="rcpickStar<%=p%>" class="btn btn-lg myrecipick" onclick="Insertmyrecipe(<%=vo.getRcp_code()%>)">Pick등록 <i class="fas fa-star"></i></button>
						<%}else{ %>
						<button id="rcpickCheck<%=p%>" class="btn btn-lg myrecipick" onclick="Deletmyrecipe(<%=vo.getRcp_code()%>)">Pick해제 <i class="fas fa-check"></i></button>
           <%} %>
            <%}
						p++;
						%>
